...
from utils.helper_utils import execute_command, read_file
...
def start_docker_compose(logger):
    if details["start_docker_compose"]:
        try:
            ...
            execute_command(f"sshpass -p {details["password_for_sshpass"]} sudo docker-compose -f {Fc.docker_compose_file} up -d")
...
def all_services_healthy():
    result = execute_command(f"sshpass -p {details["password_for_sshpass"]} sudo docker-compose -f {Fc.docker_compose_file} ps")
